...
def opticalFlow(old_frame_pyramid, new_frame_pyramid, r, ksize = 17):
  pass
...
#def opticalFlow(im1: np.ndarray, im2: np.ndarray, step_size=10, win_size=5) -> (np.ndarray, np.ndarray):
    ...
    #         z = numpy.zeros((max(mat.shape), max(mat.shape), max(mat.shape)))
    #         z[:mat.shape[0], :mat.shape[1]] = mat
    #         mat = z
    #         l1 = np.sort(np.linalg.eig(mat)[0])[0]
    #         l2 = np.sort(np.linalg.eig(mat)[0])[1]
    #         if (l2[0] > 1 and l1[0]/l2[0] < 100):
...
def sumWindow (arr: np.ndarray, x: int, y: int, win_size: int):
    ...
    for row in range(len(window) - 1):
        for col in range(len(window[0]) - 1):
            sum =+ arr[row,col]
    return sum
...
def expand(im, shape, filter_vec):
    ...
    zeros[::2, ::2] += im
    return blur(zeros, 2*filter_vec)
...
def laplaceianExpand(lap_pyr: List[np.ndarray], coeff, filter_vec) -> np.ndarray:
    ...
    image = lap_pyr[SMALLEST_PYR]
    for i in range(len(lap_pyr) - 1, LARGEST_PYR, REVERSED):
        image = expand(image, lap_pyr[i - 1].shape, filter_vec) + lap_pyr[i - 1]
...
def expand_im(im, g_kernel):
    ...
    toReturn = blur(padded_im, g_kernel * 2)
    return toReturn
...
def pyrBlend(img_1: np.ndarray, img_2: np.ndarray, mask: np.ndarray, levels: int) -> (np.ndarray, np.ndarray):
    ...
    blur = blur_image(img_1, filter_vec)
    return blend_im.clip(0, 1), blur
